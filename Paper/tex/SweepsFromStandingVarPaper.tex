...
We can then consider a neutral locus some genetic distance $r$ away from the beneficial mutation, and ask what is the probability that a recombination event moves the allele at this neutral locus from the selected background into the non selected background at some point during the course of the sweep. The probability of a recombination event in a given generation is just $r$, but we are only interested recombination events that move our neutral locus onto a non-beneficial background. If the frequency of the beneficial allele at generation $t$ in the past is $X\left(t\right)$, then the probability that a neutral lineage which has failed to recombine up to generation $t$ does so in generation $t$ is $r\left(1-X\left(t\right)\right)$, and the probability that it fails to recombine at any point during the course of the sweep is approximately
\begin{equation}
	\prod_{X\left(t\right)=1-\frac{1}{2N}}^{\frac{1}{2N}} 1-r\left(1-X(t)\right)  \approx \exp \left(-r \int_{1-\frac{1}{2N}}^{\frac{1}{2N}}(1-X\left(t\right))\mathrm{d} X\left(t\right) \right) \approx \exp \left(-rT_{fix}\right) \approx \exp \left(-\frac{r}{s}2ln\left(2N\right)\right). \label{simple-sweep-approx}
\end{equation}

We can further improve this approximation by recognizing that conditional on reaching fixation, adaptive alleles are likely to have undergone a brief period of very rapid increase from a frequency of $\frac{1}{2N}$ up to $\frac{1}{2Ns}$ to escape loss. We can therefore assume that all recombination occurs when the beneficial allele is at a frequency greater than $\frac{1}{2Ns}$, and coalescence occurs instantaneously once the allele dips below this frequency (backward in time). The effect of this correction is simply that we replace $2N$ in the natural logarithm in \eqref{simple-sweep-approx} with $2Ns$.
