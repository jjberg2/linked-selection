...
Next, we need to simulate the distance, $r_3$, to the recombination event which splits our sample from 2 into 3 haplotypes. One approach is to again simulate an $\exp\left(r_{BP}\widetilde{T}\right)$ distance and drop a recombination event uniformly on the branches of the tree, but this time we only keep the event if it occurs in a place that would cause a visible change to the haplotype partition. Otherwise we simulate another exponential distance and try again. An alternate approach is to first prune the tree of branches where a recombination event would not affect the partition, calculate the total time in this pruned tree (call it $\widetilde{T}_{-2}$), then simulate an $\exp\left(r_{BP}\widetilde{T}_{-2}\right)$ distance and drop a recombination event uniformly on the unpruned branches. If the recombination event at $r_2$ was due to the sweep phase, then this new recombination event is either another singleton (with probability $\frac{\left(n-1\right)\tau_f}{\widetilde{T}_{-2}}$), or partitions the remaining $n-1$ lineages according to the remaining $n-1$ lineages according the Ewens Sampling Formula conditional on k=2. However, if the event at $r_2$ occurred during the standing phase 
