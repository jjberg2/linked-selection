...
Once again, this expression is accurate far from the selected site, but in error at closely linked sites due to the contribution of new mutations. Again, we can develop an $\textit{ad hoc}$ approximation to incorporate new mutations (see appendix and Figure \ref{freq_spec}), but the approximation is not quite as good as for the no sweep case, principally because it fails to fully account for new singleton mutations occurring during the sweep phase. While further extensions of our approximation could be developed to accommodate these singletons, the expressions are already quite unwieldy, so we do not pursue them further here.
...
\begin{equation}
	q_{new}(l \mid k, i, n) =
	\begin{cases} 
		\theta f + \mu n\tau_f, 	& \text{if } i = n, k = 1, l=1 \\
		\mu i\tau_f,			& \text{if } i < n, k = 1, l =1 \\
...
\begin{multline}
		...
		\sum_{j=1}^{\text{min}\left(k+n-i-1,\ell\right)} \left(q_{anc}(j\mid k+n-i)\sum_{g = \text{max} \left( j - k , 0 \right) }^{\text{min} \left( j , \ell , \left(n-i\right) \right)} H(g \mid j,k,n-i) p(\ell-g \mid j-g,k,i) + q_{new}(l \mid k,i,n)\right) 
\end{multline}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
