...
An alternate possibility which seems potentially fruitful is to identify a set of statistics which can jointly both distinguish swept regions from non-swept regions, and distinguish among different kinds of sweeps. For this task, we follow previous authors \citep{Garud:2015jy} by examining the extent to which the ordered haplotype frequency spectrum (with haplotypes now defined via IBS) can be useful for identifying and distinguishing different varieties of sweeps.

For a window of size $L$, we define the the ordered haplotype frequency spectrum (OHFS) as $\mathcal{H}_L = \{h_1,h_2,\dots,h_{P}\}$, where $h_p$ gives the frequency of the $p^{th}$ most common haplotype and there are a total of $P$ distinct haplotypes within the window. As an example of how features of the OHFS can be used both to identify and distinguish between different kinds of sweeps, consider the methods proposed by \cite{Garud:2015jy} as a means to identify multiple mutation soft sweeps. \cite{Garud:2015jy} define the following set of summary statistics
\begin{align}
	H_1 &= \sum_{i=1}^{P}h_{i}^2 \\
	H_2 &= \sum_{i=2}^P h_{i}^2 \\
	H_{12} &= \left(h_1 + h_2\right)^2 + \sum_{i=3}^P h_{i}^2 \\
	H_{123} &= \left(h_1 + h_2 + h_3\right)^2 + \sum_{i=4}^P h_{i}^2	\\
	&\text{and} \\
	H_2/H_1 &= \frac{\sum_{i=2}^Ph_i^2}{\sum_{i=1}^Ph_i^2}
\end{align}
with the rationale that both hard sweeps are characterized by a single core haplotype at high frequency, whereas soft sweeps are characterized by a small set of core haplotypes, each individually at intermediate frequency, but collectively constituting most of the population.

One of the most easily noticeable factors which distinguishes the OHFS of a standing sweep from that of a hard sweep is that for a window of a given size there will tend to be more distinct haplotypes for a standing sweep than for a hard sweep. This is purely due to the fact that the there is more time in the genealogy at the selected site for a standing sweep than for a hard sweep of equal strength, giving more opportunity for both recombination events and new mutations to create new haplotypes. Given that a new haplotypes has been created, it is most likely to have been created by budding off from the most frequent haplotype. In \jb{figure BLAH} we show from simulations $P\left(\lvert \mathcal{H}_L^{stand} \rvert \geq i \right)/P\left(\lvert \mathcal{H}_L^{hard} \rvert \geq i \right)$ over a range of value of $L$ for one sided windows extending away from the selected site (where $\lvert \cdot \rvert$ denotes the length of the vector). As expected, we see that the number of haplotypes increases more quickly with distance from the selected site for a standing sweep than for a hard sweep. This is in line with our results regarding the recovery of genetic diversity surrounding a standing sweep, which indicate that to first order \jb{?} a sweep from standing variation simply tends to look like a sweep with a weaker selection coefficient.

An alternative source of information from the OHFS which we could use to distinguish different varieties of sweeps is that, conditional on existing, the relative frequency of the $i^{th}$ most common haplotype in a given window size differs depending on the sweep model. For example, it is well known that for a multiple mutation soft sweep (with, say, three origins of the beneficial allele), there will be a small window surrounding the selected site where the most common haplotype will tend to be less frequent than the most common haplotype for a hard sweep of equal strength, while the second and third most frequent haplotypes will tend to be at higher frequencies than the second and third most common haplotypes generated during a hard sweep.

In \jb{figure BLAH}, we show $\mathbb{E}[h_i^{M1} \mid i > 0]/\mathbb{E}[h_i^{M2} \mid i > 0]$ where $M1$ and $M2$ denote different models of sequence evolution. In particular, we note that 


and $\mathbb{E}[h_i^{soft}]/\mathbb{E}[h_i^{hard}]$ (conditional on $h_i \neq 0$) in a sample of $n=100$ over a broad range of window sizes and for the multiple mutation soft sweep have conditioned on three origins of the beneficial allele. 

The other major distinguishing factor between a standing sweep and a hard sweep is that the genealogy for a hard sweep has most of its branch length near the tips, whereas a standing sweep has a genealogy that more closely resembles the neutral coalescent. This means that in general, we should expect the $i^{th}$ most common haplotype for a given window size to be more frequent if a sweep came from standing variation than if it came from a \textit{de novo} mutation, and this should hold over a large portion of the OHFS. 


As indicated by \citep{Pennings2006}, the major feature which distinguishes a multiple mutation soft sweep from a classic hard sweep is the fact that the second and third most frequent haplotypes are much more common in former than in the latter. We also observe that there is an inflation of $h_{4}-h_{20}$ relative to a hard sweep as the window size expands, but this is predominantly a result of the fact than a haplotype which would have been the second most common in a hard sweep is now the fourth most common in the soft sweep case, and is thus at a higher frequency than the fourth most common haplotype created under a standing sweep. We suspect there is therefore relatively little information to identify a soft sweep beyond the detection of the multiple core haplotypes carrying the original beneficial mutation.
...
Despite our inability to deal with the process analytically, we can use intuition gained from our description of it to make a few predictions regarding features of the haplotype transition scheme and check whether they are born out in simulations. The principal difference in terms of the shape of the genealogy between a hard sweep and a standing sweep is that for a hard sweep nearly all of the branch length in the tree is near the tips (i.e. singleton lineages or branches that subtend small families of lineages), in contrast to a standing sweep, which has internal branch structure similar to the neutral coalescent. This means that in a hard sweep, most new haplotypes are created by budding off of low frequency haplotypes (often singletons) from the dominant core haplotype, whereas under a standing sweep, we expect to more commonly observe one of the few most frequent haplotypes to split into two haplotypes of a more intermediate frequency. 
...
%
%We begin at the locus of the beneficial allele, and initiate the process by sampling a genealogy, $\mathscr{T}$, by taking a sample from the neutral coalescent with a population size of $2Nf$, and then adding a length of $\tau_f$ to each external branch, corresponding to the sweep phase. At the selected site, all $n$ lineages belong to a single identity-by-descent partition, $\mathbb{P}_1$. Moving away from the selected site, we encounter recombination events which move lineages off of the selected background and into the ancestral population. In general, after we have encountered $i-1$ recombination events, our lineages will be split into a partition $\mathbb{P}_{i}$ of $i$ sets of lineages, each representing a collection of lineages with are identical-by-descent with respect to the beginning of the sweep. Given knowledge of the tree, $\mathscr{T}$, as well as the recombination events $\mathbb{R}_1 \dots \mathbb{R}_{i-1}$, the probability of a further particular transition in the partition scheme $\mathbb{P}_{i} \rightarrow \mathbb{P}_{i+1}$ occurs at a particular distance $r_{i}$ past the previous recombination event can be calculated as follows. Identify and remove all branches in $\mathscr{T}$ where a further recombination event would cause no change in the partition scheme, leaving behind a set of pruned subtrees, $\mathscr{T}^{-i}$. Let the total time in this collection of pruned subtrees be $\widetilde{T}^{-i}$, and let $t_{\mathbb{P}_{i+1}}$ represent the total length of branches on which a recombination event would give the desired transition in the partition scheme. Given these quantities, the probability of a particular transition is
%$$P\left(\mathbb{P}_i \rightarrow \mathbb{P}_{i+1} \mid \mathbb{R}_1 \dots \mathbb{R}_{i-1},\mathscr{T} \right) = \frac{t_{\mathbb{P}_{i+1}}}{\widetilde{T}^{-i}}$$
%and the probability that this event occurs at a distance $r_{i}$ is
%$$P\left(r_{i} \mid \mathbb{P}_i \rightarrow \mathbb{P}_{i+1}, \mathbb{R}_1 \dots \mathbb{R}_{i-1},\mathscr{T} \right) = r_{BP}e^{-r_{BP}t_{\mathbb{P}_{i+1}}}$$
%and so the probability that a particular set of transitions in the IBD partition occurs at a particular set of distances, conditional on the genealogy at the selected site
%$$\prod_{i = 1}^{n-1}P\left(r_{i} \mid \mathbb{P}_i \rightarrow \mathbb{P}_{i+1}, \mathbb{R}_1 \dots \mathbb{R}_{i-1},\mathscr{T} \right)P\left(\mathbb{P}_i \rightarrow \mathbb{P}_{i+1} \mid \mathbb{R}_1 \dots \mathbb{R}_{i-1},\mathscr{T} \right)$$
%
%

%
%An understanding of these patterns would be particularly useful in conjunction with a description of the patterns expected under other sweep models, such as the classic hard sweep model, and the multiple mutation soft sweep model. Below, we describe how one would go about simulating the changes in the IBD partition along the sequence under our model, and along the way consider the quantities one would need to calculate in order to  of how the haplotype partition changes with distance from the selected locus under our standing sweep model
%
%, and then compare this with the patterns expected under the alternate models and neutrality.
%
%One useful way to gain some intuition about patterns of haplotypic variation along the sequence is to consider simulating the process. We begin by simulating a neutral genealogy in a population of $2Nf$ corresponding to the standing phase, and then adding an extra length of $\tau_f$ to each external branch, corresponding to the sweep phase. Let $\widetilde{T}_{f}$ denote the total time in the tree from the period of time prior to the onset of selection, and let $\widetilde{T} = \widetilde{T}_{f} + n\tau_f$ give the total time in the tree including the sweep phase. $\widetilde{T}_{f}$
%
%
%Next, we begin adding recombination events which partition the sample into more and more smaller and smaller haplotypes as we move away from the selected site. 
%
%Let $r_2$ denote the distance in base pairs to the first recombination event which partitions the sample into two distinct haplotypes which do not share identity by descent with respect to the sweep.  We then have $r_2 \sim \exp\left(r_{BP}\widetilde{T}\right)$, and we place the event uniformly on the branches of our small coalescent tree. Absent knowledge of the underlying tree, our approximation suggests this event either creates a singleton due to a recombination event during the sweep phase (with probability $\frac{n\tau_f}{\widetilde{T}}$), or partitions the sample according to the the Ewens Sampling Formula conditional on $k = 2$.
%
%Next, we need to simulate the distance, $r_3$, to the recombination event which splits our sample from 2 into 3 haplotypes. One approach is to again simulate an $\exp\left(r_{BP}\widetilde{T}\right)$ distance and drop a recombination event uniformly on the branches of the tree, but this time we only keep the event if it occurs in a place that would cause a visible change to the haplotype partition. Otherwise we simulate another exponential distance and try again. An alternate approach is to first prune the tree of branches where a recombination event would not affect the partition, calculate the total time in this pruned tree ($\widetilde{T}_{-2}$), then simulate an $\exp\left(r_{BP}\widetilde{T}_{-2}\right)$ distance and drop a recombination event uniformly on the unpruned branches. If the recombination event at $r_2$ was due to the sweep phase, then this new recombination event is either another singleton (with probability $\frac{\left(n-1\right)\tau_f}{\widetilde{T}_{-2}}$), or else partitions the remaining $n-1$ lineages according to the Ewens Sampling Formula conditional on k=2. If the event at $r_2$ occurred during the standing phase, then if the event at $r_3$ occurred during the sweep phase it simply removes a single lineage from one of the two core haplotypes and makes it a singleton. However, if the $r_3$ event also occurred during the standing phase, then without knowledge of the tree (which we do not have in practice), we are at an impasse. This is because a calculation of the distribution on $r_3$, as well as the partition created by this event, requires that we sum over all possible trees which contain a mutation that would yield the relevant partition at $r_2$, a task that is beyond our mathematical means, and to our knowledge has not been accomplished. Given that, in the standard Ewens Sampling Formula, the number of alleles is a sufficient statistic to infer the clustering parameter, one might naively hope that conditional on the event $r_2$, all additional information obtained from the event at $r_3$ is contained in its distance (i.e. that the specific details of the partition are irrelevant). Unfortunately, this is not the case (pers. comm., J. Hermisson). If we know how the first recombination event divides our sampled lineages at the first event, the information from the second event is present in both the distance and the further partitioning, and both of these depend on the partition at the first event.
%
%
%
