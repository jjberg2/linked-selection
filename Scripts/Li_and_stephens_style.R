...
fs<-seq(.005,.2,length=30)
for(tree.index in 1:20){
tree<-temp$trees[[tree.index]]
perm.like<-calc.like(tree,fs)
...
for(perm in 1:10){
	...
	for(f.index in 1:length(fs)){
		...
		hap.probs<-sapply(1:(num.haps-1),function(k){
			...
			hap.prob<-calc.prob.new.hap(k,new.hap,old.haps,rec.loc,rec.param)
			return(hap.prob)
		})
...
calc.prob.new.hap<-function(k,new.hap,old.haps,rec.loc,rec.param){
