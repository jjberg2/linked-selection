...
plot ( c ( 0 , 200 ) , c ( 0 , 1 ) , type = "n" , xlab = "4NR" , ylab = expression ( pi [ R ] / pi [ 0 ] ) , cex.lab = 1.5 , bty = "n" )
pi.over.f<-list()
...
pdf ("../Paper/Paper_Figures/pi_density.pdf" , width = 5 , height = 5  )
plot ( c ( 0 , 200 ) , c ( 0 , 1 ) , type = "n" , xlab = "4NR" , ylab = expression ( pi [ R ] / pi [ 0 ] ) , cex.lab = 1.5 , bty = "n" )
for ( i in 1 : length ( real.fs ) ) {
		#run.ms.f(f.index)
		# mut.density <- get.mut.density ( file = paste ( "mssel_f2" , f.index , ".out" , sep = "" ) )
...
legend ( x = 50 , y = 0.2 , legend = c ( "Standard" , "Ours" , "Simulation") , lty = c ( 3 , 2 , 1 ) , lwd = 1.5 , bty = "n")
dev.off()
