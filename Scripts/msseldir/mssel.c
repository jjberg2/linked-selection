...
main(argc,argv)
        ...
        char *argv[];
{
  int i, k, howmany, segsites,pop, oldnfreqder ;
...
/*  Since the above bit is only done for count = 0, can't use tbs argument for argument of -s switch */
	...
	fscanf(pars.selfile," n: %d",&(pars.cp.nfreqder) );	 
    ...
    while( howmany-count++ ) {
	   ...
	   if( (count>1)&& ( pars.cp.nrepsfreq >1) ){
		  oldnfreqder = pars.cp.nfreqder ;
	   	  if( fscanf(pars.selfile," n: %d",&(pars.cp.nfreqder) ) != 1 ){
		     rewind(pars.selfile);
			 do {
                 fgets( line, 1000, pars.selfile );
             }while ( line[0] == '#' );

		     fscanf(pars.selfile," %*d");
		     fscanf(pars.selfile," n: %d", &(pars.cp.nfreqder) ) ;
			}
  ...
  if( count == 0 ) {
	...
	*phowmany = atoi( argv[2] );
	...
	pars.selfile = fopen(argv[5],"r");
       	if( pars.selfile  == NULL ) {fprintf(stderr," no sel file %s\n", argv[5] ); exit(0);}
	do {
       fgets( line, 1000, pars.selfile );
    }while ( line[0] == '/' );

	sscanf(line," %d",&(pars.cp.nrepsfreq) ); 
	fscanf(pars.selfile," %d",&(pars.cp.maxpop) ); 
